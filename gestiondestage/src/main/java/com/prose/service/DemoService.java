package com.prose.service;


import com.prose.entity.*;
import com.prose.entity.users.*;
import com.prose.repository.*;
import com.prose.service.Exceptions.*;
import com.prose.service.dto.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.mock.web.MockMultipartFile;
import org.springframework.security.crypto.password.PasswordEncoder;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import org.springframework.web.multipart.MultipartFile;

import java.io.IOException;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.time.LocalTime;
import java.util.*;

@Service
class DemoService {
    private static final Logger logger = LoggerFactory.getLogger(JobOfferService.class);
    private final CurriculumVitaeService curriculumVitaeService;
    private final CurriculumVitaeRepository curriculumVitaeRepository;

    private final StudentService studentService;
    private final StudentRepository studentRepository;
    private final EmployeurService employeurService;
    private final EmployeurRepository employeurRepository;

    private final JobOfferService jobOfferService;
    private final JobOfferRepository jobOfferRepository;
    private final TeacherService teacherService;

    private final TeacherRepository teacherRepository;

    private final ProgramManagerService programManagerService;
    private final ProgramManagerRepository programManagerRepository;
    private final AcademicSessionRepository academicSessionRepository;

    private final ContractService contractService;

    private final ScheduledTasksService scheduledTasksService;

    private final PasswordEncoder passwordEncoder;
    private final LoginService loginService;


    public DemoService(CurriculumVitaeService curriculumVitaeService, CurriculumVitaeRepository curriculumVitaeRepository,
                       StudentService studentService, StudentRepository studentRepository,
                       EmployeurService employeurService, EmployeurRepository employeurRepository,
                       JobOfferService jobOfferService, JobOfferRepository jobOfferRepository,
                       TeacherService teacherService, TeacherRepository teacherRepository, ProgramManagerService programManagerService,
                       ProgramManagerRepository programManagerRepository, AcademicSessionRepository academicSessionRepository, ContractService contractService, ScheduledTasksService scheduledTasksService, PasswordEncoder passwordEncoder, LoginService loginService) throws Exception {
        this.curriculumVitaeService = curriculumVitaeService;
        this.curriculumVitaeRepository = curriculumVitaeRepository;
        this.studentService = studentService;
        this.studentRepository = studentRepository;
        this.employeurService = employeurService;
        this.employeurRepository = employeurRepository;
        this.jobOfferService = jobOfferService;
        this.jobOfferRepository = jobOfferRepository;
        this.teacherService = teacherService;
        this.teacherRepository = teacherRepository;
        this.programManagerService = programManagerService;
        this.programManagerRepository = programManagerRepository;
        this.academicSessionRepository = academicSessionRepository;
        this.contractService = contractService;
        this.scheduledTasksService = scheduledTasksService;
        this.passwordEncoder = passwordEncoder;
        this.loginService = loginService;
        doDemoService();
    }

    @Transactional
    protected void doDemoService() throws IOException, DisciplineNotFoundException, JobNotFoundException, InvalidUserFormatException, AlreadyExistsException, TooManyApplicationsExceptions, MissingPermissionsExceptions, MaxInternsReachedException, InternshipNotFoundException, DateExpiredException, InternshipOfferAlreadyConfirmedException, JobApplicationNotFoundException, InvalidBase64Exception {

        // blocking daily operations at startup
        this.scheduledTasksService.setShouldRunDailyOperationsAtStartup(false);

        // create a program manager
        ProgramManager programManager = new ProgramManager();
        programManager.setNom("Tremblay");
        programManager.setPrenom("Didier");
        programManager.setCredentials("gs@email.com",
                passwordEncoder.encode("123456"));
        programManager.setAdresse("1000 ici there");
        programManager.setTelephone("1234567890");
        programManager.setState(UserState.DEFAULT);
        programManagerRepository.save(programManager);
        UserDTO programManagerDTO = UserDTO.toDTO(programManager);

/*
        // create a valid cv for a student
        Student etudiant = new Student();
        etudiant.setNom("Marien");
        etudiant.setPrenom("Nathan");
        etudiant.setAdresse("1111 Rue Lapierre, LaSalle, QC H8N 2J4");
        etudiant.setTelephone("1234567890");
        etudiant.setDiscipline(Discipline.INFORMATIQUE);
        etudiant.setCredentials("stu1@email.com", passwordEncoder.encode("123456"));
        etudiant.setState(UserState.DEFAULT);
        etudiant.setLastConnection(LocalDateTime.now());

        Student etudiant2 = new Student();
        etudiant2.setNom("Shi");
        etudiant2.setPrenom("Bing");
        etudiant2.setAdresse("1111 Rue Lapierre, LaSalle, QC H8N 2J4");
        etudiant2.setTelephone("1234567890");
        etudiant2.setDiscipline(Discipline.INFORMATIQUE);
        etudiant2.setCredentials("stu2@email.com", passwordEncoder.encode("123456"));
        etudiant2.setLastConnection(LocalDateTime.now());
        etudiant2.setState(UserState.DEFAULT);


        Student etudiant3 = new Student();
        etudiant3.setNom("Wang");
        etudiant3.setPrenom("Renfeng");
        etudiant3.setAdresse("1111 Rue Lapierre, LaSalle, QC H8N 2J4");
        etudiant3.setTelephone("1234567890");
        etudiant3.setDiscipline(Discipline.INFORMATIQUE);
        etudiant3.setCredentials("stu3@email.com", passwordEncoder.encode("123456"));
        etudiant3.setState(UserState.DEFAULT);

        Student etudiant4 = new Student();
        // random
        etudiant4.setNom("Cardin");
        etudiant4.setPrenom("Alexis");
        etudiant4.setAdresse("1111 Rue Lapierre, LaSalle, QC H8N 2J4");
        etudiant4.setTelephone("1234567890");
        etudiant4.setDiscipline(Discipline.INFORMATIQUE);
        etudiant4.setCredentials("stu4@email.com", passwordEncoder.encode("123456"));
        etudiant4.setState(UserState.DEFAULT);

        Student stu1BD = studentRepository.save(etudiant);
        Student stu2DB = studentRepository.save(etudiant2);
        Student stu3DB = studentRepository.save(etudiant3);
        Student stu4DB = studentRepository.save(etudiant4);

        MultipartFile file = loadMockMultipartFile("CV_test_stu1.pdf", "/Nathan_Marien_CV_2024_v2.pdf");
        CurriculumVitaeDTO cvdto = curriculumVitaeService.addCV(file, LocalDateTime.now(), "stu1@email.com");
        MultipartFile file2 = loadMockMultipartFile("CV_test_stu2.pdf", "/Nathan_Marien_CV_2024_v2.pdf");
        CurriculumVitaeDTO cvdto2 = curriculumVitaeService.addCV(file2, LocalDateTime.of(2024, 10, 1, 0, 0), "stu2@email.com");
        MultipartFile file4 = loadMockMultipartFile("CV_test_stu3.pdf", "/Nathan_Marien_CV_2024_v2.pdf");
        CurriculumVitaeDTO cvdto3 = curriculumVitaeService.addCV(file4, LocalDateTime.of(2024, 10, 1, 0, 0), "stu3@email.com");
        MultipartFile file7 = loadMockMultipartFile("CV_test_stu4.pdf", "/Nathan_Marien_CV_2024_v2.pdf");
        CurriculumVitaeDTO cvdto4 = curriculumVitaeService.addCV(file7, LocalDateTime.of(2024, 10, 1, 0, 0), "stu4@email.com");
        MultipartFile file6 = loadMockMultipartFile("CV_test_stu1.pdf", "/Nathan_Marien_CV_2024_v2.pdf");
        CurriculumVitaeDTO cvdto6 = curriculumVitaeService.addCV(file6, LocalDateTime.of(2024, 10, 1, 0, 0), "stu1@email.com");

        // valide cv
        curriculumVitaeService.validateCV(cvdto.id(), "validated",programManagerDTO);
        curriculumVitaeService.validateCV(cvdto2.id(), "validated",programManagerDTO);
        curriculumVitaeService.validateCV(cvdto3.id(), "validated",programManagerDTO);
        curriculumVitaeService.validateCV(cvdto4.id(), "validated",programManagerDTO);


        // create a valid job offer
        Employeur employeur = new Employeur();
        employeur.setNomCompagnie("Hydro-Québec");
        employeur.setContactPerson("Alice");
        employeur.setAdresse("123 rue de la compagnie");
        employeur.setCity("Montréal");
        employeur.setPostalCode("H1H1H1");
        employeur.setTelephone("1234567890");
        employeur.setFax("1234567890");
        employeur.setCredentials("emp@email.com",passwordEncoder.encode("123456"));
        employeur.setState(UserState.DEFAULT);
        Employeur empDB = employeurRepository.save(employeur);


        MultipartFile file3 = loadMockMultipartFile("JobOffer_test.pdf", "/Offre_1475.pdf");
        JobOfferRegisterDTO registerDTO = new JobOfferRegisterDTO("Python Developer", "2025-01-01", "2025-05-31", "1111 Rue Lapierre, LaSalle, QC H8N 2J4", "remote", 3, 20.5, 32.0,
                LocalTime.of(8, 0), LocalTime.of(16, 0),
                "Python Developer" );
        JobOfferDTO jobOfferDTO = jobOfferService.createJobOffer(registerDTO, file3, empDB.getEmail());

        MultipartFile file5 = loadMockMultipartFile("JobOffer_test.pdf", "/Offre_1475.pdf");
        JobOfferRegisterDTO registerDTO2 = new JobOfferRegisterDTO("Python Developer", "2025-01-01", "2025-05-31", "1111 Rue Lapierre, LaSalle, QC H8N 2J4", "remote", 3, 20.5, 32.0,
                LocalTime.of(8, 0), LocalTime.of(16, 0),
                "Python Developer" );
        JobOfferDTO jobOfferDTO2 = jobOfferService.createJobOffer(registerDTO2, file5, empDB.getEmail());

        // valide job offer
        JobPermissionRegisterDTO jobPermissionRegisterDTO = new JobPermissionRegisterDTO(
                new ArrayList<>(List.of("informatique")),
                new ArrayList<>(),
                LocalDate.of(2024, 12, 31),
                true
        );
        programManagerService.createJobPermission(jobOfferDTO.id(), jobPermissionRegisterDTO,programManagerDTO);

        // create a valide application
        Long jobOfferApplicationId = null;
        try{
            JobOfferApplicationDTO jobOfferApplicationDTO = jobOfferService.applyToJobOffer(jobOfferDTO.id(),cvdto.id(),UserDTO.toDTO(stu1BD));
            jobOfferApplicationId = jobOfferApplicationDTO.id();
        }
        catch (Exception e){
            logger.error("Error creating application: {}", e.getMessage());
        }

        // create an interview
        try {
            JobInterviewRegisterDTO jobInterviewRegisterDTO = new JobInterviewRegisterDTO(
                    "2024-12-07T21:16",
                    "Online",
                    "https://meet.google.com/",
                    1L
            );
            employeurService.createJobInterview(jobInterviewRegisterDTO, UserDTO.toDTO(empDB));
        }
        catch (Exception e){
            logger.error("Error creating interview: {}", e.getMessage());
        }

        // confirm internship from employer side
        InternshipOfferDTO internshipOfferDTO;
        try {
            if (jobOfferApplicationId != null)
                jobOfferService.offerInternshipToStudent(jobOfferApplicationId, UserDTO.toDTO(empDB), 30L);
            else
                logger.error("Error confirming internship from employer side: jobOfferApplicationId is null");
        }
        catch (Exception e){
            logger.error("Error confirming internship from employer side: {}", e.getMessage());
        }


        // create 2 profs

        TeacherRegisterDTO teacherRegisterDTO = new TeacherRegisterDTO("Lacoursière", "François",
                "prof1@email.com ", "Montréal","1234567890", "123456",
                "informatique");
        TeacherDTO teacherDTO = teacherService.createTeacher(teacherRegisterDTO);

        TeacherRegisterDTO teacherRegisterDTO2 = new TeacherRegisterDTO("Lacoursièrette", "Francine",
                "prof2@email.com ", "Montréal","1234567890", "123456",
                "informatique");
        TeacherDTO teacherDTO2 = teacherService.createTeacher(teacherRegisterDTO2);

        // signable contract
        String signStudent = "";
        String signGS = "";
        String signEmp = "";
        //String signature = "";

        // create fully prepared internship evaluation with stu3 assigned to prof2
        // internship starts today to test notification
        Optional<Teacher> teacher2 = teacherRepository.findUserAppByEmail("prof2@email.com");
        if (teacher2.isPresent()) {
            // creating valid job offer that starts today
            String dateDebut = LocalDate.now().toString();
            String dateFin = LocalDate.now().plusMonths(4).toString();
            JobOfferRegisterDTO registerDTO3 = new JobOfferRegisterDTO("Another Python Developer", dateDebut, dateFin, "1111 Rue Lapierre, LaSalle, QC H8N 2J4", "remote", 3, 20.5, 32.0,
                    LocalTime.of(8, 0), LocalTime.of(16, 0),
                    "Another Python Developer" );
            JobOfferDTO jobOfferDTO3 = jobOfferService.createJobOffer(registerDTO3, file3, empDB.getEmail());

            JobPermissionRegisterDTO jobPermissionRegisterDTO3 = new JobPermissionRegisterDTO(
                    new ArrayList<>(List.of("informatique")),
                    new ArrayList<>(),
                    LocalDate.of(2024, 12, 31),
                    true
            );
            programManagerService.createJobPermission(jobOfferDTO3.id(), jobPermissionRegisterDTO3,programManagerDTO);

            // stu3 apply to job offer
            Long jobOfferApplicationId2 = null;
            try{
                JobOfferApplicationDTO jobOfferApplicationDTO = jobOfferService.applyToJobOffer(jobOfferDTO3.id(),cvdto3.id(),UserDTO.toDTO(stu3DB));
                jobOfferApplicationId2 = jobOfferApplicationDTO.id();
            }
            catch (Exception e){
                logger.error("Error creating application: {}", e.getMessage());
            }

            // confirm internship from employer side
            InternshipOfferDTO internshipOfferDTO2 = null;
            try {
                if (jobOfferApplicationId2 != null)
                    internshipOfferDTO2 = jobOfferService.offerInternshipToStudent(jobOfferApplicationId2, UserDTO.toDTO(empDB), 1000000L);
                else
                    logger.error("Error confirming internship from employer side: jobOfferApplicationId2 is null");
            }
            catch (Exception e){
                logger.error("Error confirming internship from employer side: {}", e.getMessage());
            }

            // confirm internship from student side
            try {
                if (internshipOfferDTO2 != null)
                    jobOfferService.confirmInternshipOffer(internshipOfferDTO2.id(), "accepted", stu3DB.getId());
                else
                    logger.error("Error confirming internship from student side: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error confirming internship from student side: {}", e.getMessage());
            }

            // start contract
            try {
                if (internshipOfferDTO2 != null)
                    programManagerService.beginContract(internshipOfferDTO2.id(),programManagerDTO);
                else
                    logger.error("Error starting contract: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error starting contract: {}", e.getMessage());
            }

            // make each party sign
            try {
                if (internshipOfferDTO2 != null) {
                    contractService.sign(new UserDTO(programManager.getId(), programManager.getEmail(), programManager.getPassword(), Role.PROGRAM_MANAGER), internshipOfferDTO2.id(), new SignatureBase64DTO(signGS, programManager.getPassword()));
                    contractService.sign(new UserDTO(stu3DB.getId(), stu3DB.getEmail(), stu3DB.getPassword(), Role.STUDENT), internshipOfferDTO2.id(), new SignatureBase64DTO(signStudent, stu3DB.getPassword()));
                    contractService.sign(new UserDTO(empDB.getId(), empDB.getEmail(), empDB.getPassword(), Role.EMPLOYEUR), internshipOfferDTO2.id(), new SignatureBase64DTO(signEmp, empDB.getPassword()));
                }
                else
                    logger.error("Error signing contract: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error signing contract: {}", e);
            }

            // assign renfeng to prof2
            try {
                if (internshipOfferDTO2 != null && teacher2.isPresent()) {
                    programManagerService.assignToProf(teacherDTO2.id(), internshipOfferDTO2.id(), programManagerDTO);
                }
                else
                    logger.error("Error assigning teacher to student: internshipOfferDTO2 or teacher is null");
            }
            catch (Exception e){
                logger.error("Error assigning teacher to student: {}", e);
            }
        }

        // create fully prepared internship evaluation with stu4 assigned to prof2
        // internship ends in 2 weeks to test notification
        if (teacher2.isPresent()) {
            // creating valid job offer that ends in 2 weeks
            String dateDebut = LocalDate.now().minusWeeks(5).toString();
            String dateFin = LocalDate.now().plusWeeks(2).toString();
            JobOfferRegisterDTO registerDTO3 = new JobOfferRegisterDTO("Very Cool Java Developer", dateDebut, dateFin, "1111 Rue Lapierre, LaSalle, QC H8N 2J4", "remote", 3, 20.5, 32.0,
                    LocalTime.of(8, 0), LocalTime.of(16, 0),
                    "Very Cool Java Developer" );
            JobOfferDTO jobOfferDTO3 = jobOfferService.createJobOffer(registerDTO3, file3, empDB.getEmail());

            JobPermissionRegisterDTO jobPermissionRegisterDTO2 = new JobPermissionRegisterDTO(
                    new ArrayList<>(List.of("informatique")),
                    new ArrayList<>(),
                    LocalDate.of(2024, 12, 31),
                    true
            );
            programManagerService.createJobPermission(jobOfferDTO3.id(), jobPermissionRegisterDTO2,programManagerDTO);

            // stu4 apply to job offer
            Long jobOfferApplicationId2 = null;
            try{
                JobOfferApplicationDTO jobOfferApplicationDTO = jobOfferService.applyToJobOffer(jobOfferDTO3.id(),cvdto4.id(),UserDTO.toDTO(stu4DB));
                jobOfferApplicationId2 = jobOfferApplicationDTO.id();
            }
            catch (Exception e){
                logger.error("Error creating application: {}", e.getMessage());
            }

            // confirm internship from employer side
            InternshipOfferDTO internshipOfferDTO2 = null;
            try {
                if (jobOfferApplicationId2 != null)
                    internshipOfferDTO2 = jobOfferService.offerInternshipToStudent(jobOfferApplicationId2, UserDTO.toDTO(empDB), 1000000L);
                else
                    logger.error("Error confirming internship from employer side: jobOfferApplicationId2 is null");
            }
            catch (Exception e){
                logger.error("Error confirming internship from employer side: {}", e.getMessage());
            }

            // confirm internship from student side
            try {
                if (internshipOfferDTO2 != null)
                    jobOfferService.confirmInternshipOffer(internshipOfferDTO2.id(), "accepted", stu4DB.getId());
                else
                    logger.error("Error confirming internship from student side: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error confirming internship from student side: {}", e.getMessage());
            }

            // start contract
            try {
                if (internshipOfferDTO2 != null)
                    programManagerService.beginContract(internshipOfferDTO2.id(),programManagerDTO);
                else
                    logger.error("Error starting contract: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error starting contract: {}", e.getMessage());
            }

            // make each party sign
            try {
                if (internshipOfferDTO2 != null) {
                    //String signature = "";
                    contractService.sign(new UserDTO(programManager.getId(), programManager.getEmail(), programManager.getPassword(), Role.PROGRAM_MANAGER), internshipOfferDTO2.id(),
                            new SignatureBase64DTO(signGS, programManager.getPassword()));
                    contractService.sign(new UserDTO(stu4DB.getId(), stu4DB.getEmail(), stu4DB.getPassword(), Role.STUDENT), internshipOfferDTO2.id(), new SignatureBase64DTO(signStudent, stu3DB.getPassword()));
                    contractService.sign(new UserDTO(empDB.getId(), empDB.getEmail(), empDB.getPassword(), Role.EMPLOYEUR), internshipOfferDTO2.id(), new SignatureBase64DTO(signEmp, empDB.getPassword()));
                }
                else
                    logger.error("Error signing contract: internshipOfferDTO2 is null");
            }
            catch (Exception e){
                logger.error("Error signing contract: {}", e);
            }

            // assign alexis to prof2
            try {
                if (internshipOfferDTO2 != null && teacher2.isPresent()) {
                    programManagerService.assignToProf(teacherDTO2.id(), internshipOfferDTO2.id(), programManagerDTO);
                }
                else
                    logger.error("Error assigning teacher to student: internshipOfferDTO2 or teacher is null");
            }
            catch (Exception e){
                logger.error("Error assigning teacher to student: {}", e);
            }
        }*/
        StudentDTO studentDTO1 = studentService.createStudent(new
                StudentRegisterDTO("James", "Noah",
                "stu2@email.com",
                "3941, rue Cmabira",
                "5552659999",
                "123456",
                Discipline.ACCOUNTING.toString()));

         CurriculumVitaeDTO curriculumVitaeDTO1= curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "noah_cv.pdf",
                        "/cvsPDF/CV_Noah.pdf"),
                LocalDateTime.now(),
                studentDTO1.courriel()
        );

         curriculumVitaeService.validateCV(curriculumVitaeDTO1.id(),
                 ApprovalStatus.VALIDATED.toString(),
                 programManagerDTO);

        StudentDTO studentDTO2 = studentService.createStudent(new
                StudentRegisterDTO("Marien", "Nathan",
                "stu3@email.com",
                "3941, rue Cmabira",
                "5552659999",
                "123456",
                Discipline.INFORMATIQUE.toString()));

        CurriculumVitaeDTO curriculumVitaeDTO2= curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now(),
                studentDTO2.courriel()
        );

        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }

        curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv_old.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now().minusDays(1),
                studentDTO2.courriel()
        );
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv_older.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now().minusDays(1),
                studentDTO2.courriel()
        );
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv_oldest.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now().minusDays(1),
                studentDTO2.courriel()
        );
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv_even_more_old.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now().minusDays(1),
                studentDTO2.courriel()
        );
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Nathan_cv_moreOld.pdf",
                        "/cvsPDF/CV_nathan.pdf"),
                LocalDateTime.now().minusDays(1),
                studentDTO2.courriel()
        );
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }

        curriculumVitaeService.validateCV(curriculumVitaeDTO2.id(),
                ApprovalStatus.VALIDATED.toString(),
                programManagerDTO);

        StudentDTO studentDTO3 = studentService.createStudent(new
                StudentRegisterDTO("Bing", "Shi",
                "stu4@email.com",
                "3941, rue Cmabira",
                "5552659999",
                "123456",
                Discipline.INFORMATIQUE.toString()));

        CurriculumVitaeDTO curriculumVitaeDTO3= curriculumVitaeService.addCV(
                loadMockMultipartFile(
                        "Shi_cv.pdf",
                        "/cvsPDF/CV_Bing.pdf"),
                LocalDateTime.now(),
                studentDTO3.courriel()
        );

        TeacherRegisterDTO teacherRegisterDTO = new TeacherRegisterDTO("Lacoursière", "François",
                "prof1@email.com ", "Montréal","1234567890", "123456",
                "informatique");
        TeacherDTO teacherDTO = teacherService.createTeacher(teacherRegisterDTO);

        TeacherRegisterDTO teacherRegisterDTO2 = new TeacherRegisterDTO("Lacoursièrette", "Francine",
                "prof2@email.com ", "Montréal","1234567890", "123456",
                "informatique");
        TeacherDTO teacherDTO2 = teacherService.createTeacher(teacherRegisterDTO2);

        curriculumVitaeService.validateCV(curriculumVitaeDTO3.id(),
                ApprovalStatus.VALIDATED.toString(),
                programManagerDTO);

        EmployeurDTO empDTO2 = employeurService.createEmployeur(new EmployeurRegisterDTO(
                "Équipe Microfix",
                "emp2@email.com",
                "Amanda",
                "Lachine",
                "K6S9V4",
                "2228974111",
                "5554444444",
                "123456",
                "123456"));

        EmployeurDTO empDTO3 = employeurService.createEmployeur(new EmployeurRegisterDTO(
                "Stelpro",
                "emp3@email.com",
                "Gregory",
                "Montréal",
                "K6S9V4",
                "2228974111",
                "5554444444",
                "123456",
                "123456"));

        EmployeurDTO empDTO4 = employeurService.createEmployeur(new EmployeurRegisterDTO(
                "Broadway Subway Constructors General Partnership",
                "emp4@email.com",
                "Pomerleau",
                "Montréal",
                "K6S9V4",
                "2228974111",
                "5554444444",
                "123456",
                "123456"));

         jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                "Administrateur de systèmes",
                "2025-01-01",
                "2025-05-25",
                "1111 Rue Lapierre, LaSalle, QC H8N 2J4",
                "hybrid",
                3,
                20.5,
                32.0,
                LocalTime.of(8, 0),
                LocalTime.of(16, 0),
                "" ),
                loadMockMultipartFile(
                        "jobOfferOffre555.pdf",
                        "/jobsPDF/job_admin_system.pdf"),
                empDTO2.courriel()
         );

        jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                        "Technicien en informatique",
                        "2025-01-10",
                        "2025-04-12",
                        "2222 Rue Normand, Laval, QC H8N 2J4",
                        "office",
                        3,
                        20.5,
                        32.0,
                        LocalTime.of(9, 0),
                        LocalTime.of(17, 0),
                        "" ),
                loadMockMultipartFile(
                        "jobOfferOffre333.pdf",
                        "/jobsPDF/job_Technicien en Informatique.pdf"),
                empDTO3.courriel()
        );

        jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                        "Systeme Analyst",
                        "2025-01-12",
                        "2025-05-02",
                        "3652 Rue Bourgeois, Laval, QC H8N 2J4",
                        "remote",
                        3,
                        20.5,
                        32.0,
                        LocalTime.of(9, 0),
                        LocalTime.of(17, 0),
                        "" ),
                loadMockMultipartFile(
                        "jobOfferOffre897.pdf",
                        "/jobsPDF/job_system analyst.pdf"),
                empDTO3.courriel()
        );

        jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                        "Systeme Installation Coordinator",
                        "2025-01-21",
                        "2025-04-26",
                        "9032 Rue Sangevoix, Longueil, QC H8N 2J4",
                        "remote",
                        2,
                        31,
                        35.0,
                        LocalTime.of(9, 0),
                        LocalTime.of(17, 0),
                        "" ),
                loadMockMultipartFile(
                        "jobOfferOffre246.pdf",
                        "/jobsPDF/job_System Installation Coordinator.pdf"),
                empDTO4.courriel()
        );

        jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                        "Java developper",
                        "2025-01-05",
                        "2025-03-19",
                        "6302 Rue Sangevoix, Longueil, QC H8N 2J4",
                        "remote",
                        2,
                        31,
                        35.0,
                        LocalTime.of(7, 0),
                        LocalTime.of(14, 0),
                        "" ),
                loadMockMultipartFile(
                        "jobOfferOffre246.pdf",
                        "/jobsPDF/job_Java developper.pdf"),
                empDTO2.courriel()
        );

         JobOfferDTO jobPython = jobOfferService.createJobOffer(new JobOfferRegisterDTO(
                        "Python Developper",
                        "2024-09-02",
                        "2024-12-02",
                        "1231 Rue Bordeau, Longueil, QC H8N 2J4",
                        "remote",
                        2,
                        31,
                        35.0,
                        LocalTime.of(7, 0),
                        LocalTime.of(14, 0),
                        "" ),
                loadMockMultipartFile(
                        "jobOfferOffre246.pdf",
                        "/jobsPDF/job_Python developper.pdf"),
                empDTO2.courriel()
        );

         JobPermissionDTO jobPermissionDTO = programManagerService.createJobPermission(jobPython.id(), new JobPermissionRegisterDTO(
                 new ArrayList<>(List.of("informatique")),
                 new ArrayList<>(),
                 LocalDate.of(2024, 12, 31),
                 true
         ), programManagerDTO);

         JobOfferApplicationDTO jobOfferAppDTO = jobOfferService.applyToJobOffer(jobPython.id(), curriculumVitaeDTO3.id(),
                 loginService.getUserByEmail(studentDTO3.courriel())
         );

        InternshipOfferDTO interDTO = jobOfferService.offerInternshipToStudent(jobOfferAppDTO.id(), loginService.getUserByEmail(empDTO2.courriel()), 30L);
        jobOfferService.confirmInternshipOffer(interDTO.id(), "accepted", studentDTO3.id());

        String signStudent = "";
        String signGS = "";
        String signEmp = "";

        programManagerService.beginContract(interDTO.id(), programManagerDTO);

        contractService.sign(loginService.getUserByEmail(studentDTO3.courriel()), interDTO.id(), new SignatureBase64DTO(signStudent, "123456"));
        contractService.sign(loginService.getUserByEmail(empDTO2.courriel()), interDTO.id(), new SignatureBase64DTO(signEmp, "123456"));
        contractService.sign(programManagerDTO, interDTO.id(), new SignatureBase64DTO(signGS, "123456"));

        programManagerService.assignToProf(teacherDTO.id(), interDTO.id(), programManagerDTO);

    }

    private MultipartFile loadMockMultipartFile(String fileName, String filePath) throws IOException {
        return new MockMultipartFile(
                "file",
                fileName,
                "application/pdf",
                Objects.requireNonNull(DemoService.class.getResourceAsStream(filePath)).readAllBytes()
        );
    }

}
